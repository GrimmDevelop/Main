<div class="well group">
    <div class="group-top">
        <button type="button" ng-click="removeHandler(group)" class="btn btn-danger btn-xs" tooltip="Remove Group" ng-disabled="notRemovable"><span class="glyphicon glyphicon-remove"></span></button>
    </div>
    <div class="form-group">
        <label for="operator" class="control-label col-md-2">Search for letters</label>
        <div class="col-md-10">
            <select class="form-control" name="operator" ng-change="onChange()" ng-model="group.properties.operator" ng-options="key as value for (key, value) in operators"></select>
        </div>
    </div>

    <div class="form-group fields" ng-repeat="field in group.fields">
        <field-row field="field" codes="codes" on-remove="onFieldRemove(field)" on-change="onChange()"></field-row>
    </div>
    <div class="form-group">
