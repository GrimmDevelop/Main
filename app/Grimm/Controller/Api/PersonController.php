...
class PersonController extends \Controller {
    ...
    protected function loadItems($totalItems, array $with = [])
    {
        ...
        if (Input::get('starts_with')) {
            $builder->where('name_2013', 'like', Input::get('starts_with') . '%');
        }
    ...
    public function update($id)
    {
        ...
        if (Input::get('name_2013') != $person->name_2013) {
            UserAction::log('persons.edit.update_field', [
                'person_id' => $person->id,
                'field' => 'name_2013',
                'data_old' => $person->name_2013,
                'data_new' => Input::get('name_2013')
            ]);
            $person->name_2013 = Input::get('name_2013');
        }

        $newAutoGenerated = (int)Input::get('auto_generated') % 2;
        if ($newAutoGenerated != $person->auto_generated) {
            UserAction::log('persons.edit.update_field', [
                'person_id' => $person->id,
                'field' => 'auto_generated',
                'data_old' => $person->auto_generated,
                'data_new' => $newAutoGenerated
            ]);
            $person->auto_generated = $newAutoGenerated;
        }
